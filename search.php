      ...
      <div class="panel">
        <div class="panel-body">
          <div class="row">    
            <br>
	<?php
	/* Query para buscar otros usuarios */
	...
	?>
	 <!-- Muestra el usuario buscado -->
		...
		<?php $query1 = $conn->query("select * from friends where my_friend_id = '$friend_id'");
			  ...
			  if ($count1 > 0){ echo 'Ya somos amigos'; }else{
		?>	
		 <!--Boton para añadir amigo-->
								<br><button  class="btn btn-info"><i class="icon-plus-sign"></i> Añadir amigo</button>
			<?php } ?>
								</div>
                  </h4></div>
				</form>
